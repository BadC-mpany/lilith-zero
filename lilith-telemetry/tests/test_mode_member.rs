...
use lilith_telemetry::{DISPATCHER, DeploymentMode, init, telemetry_event};
...
fn exhaustive_test_flock_member_mode() {
    ...
    assert!(
        ...
        "Agent Dispatcher requires connectivity binding"
    );
    ...
    match mock_collector.recv_from(&mut buffer) {
        Ok((size, addr)) => {
            println!(
                "Agent API Egress successfully verified hitting {} with {} bytes",
                addr, size
            );
