// Real integration tests for auth middleware
...
#[tokio::test]
async fn test_auth_middleware_valid_api_key_sets_extensions() {
    let yaml_content = r#"
customers:
  - api_key: "test_key_123"
    owner: "test_owner"
...
"#;
    ...
    // The important part is that auth middleware accepts it
    let request = Request::builder()
        .method("POST")
        .uri("/v1/proxy-execute")
        .header(header::CONTENT_TYPE, "application/json")
        .header("X-API-Key", "test_key_123")
        .body(Body::from(r#"{"session_id":"test","tool_name":"read_file","args":{}}"#))
        .unwrap();

    let response = app.oneshot(request).await.unwrap();
    // Should not be 401 (auth passed), but may be other error if handler logic fails
