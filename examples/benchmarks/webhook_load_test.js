...
export function handleSummary(data) {
  ...
  const getMetric = (metricName) => {
    ...
    return { avg: 0.0, min: 0.0, med: 0.0, max: 0.0, p90: 0.0, p95: 0.0, p99: 0.0 };
  };
  ...
  const targetUrl = __ENV.LILITH_URL || 'http://localhost:8080/analyze-tool-execution';
  const vus = __ENV.LILITH_VUS ? parseInt(__ENV.LILITH_VUS) : 10;
  const isRandomConv = __ENV.LILITH_RANDOM_CONV === 'true' || __ENV.LILITH_RANDOM_CONV === '1';
...
- **Throughput Mode / Lock Contention**: ${isRandomConv ? 'Randomized Sessions (Independent Storage Writes)' : 'Static Session (High Lock Contention)'}
- **Storage Tier under Test**: **${report_json.summary.storage_type}**
- **Total Requests Evaluated**: ${httpReqs.count}
...
|---|---|---|---|---|---|---|---|
| HTTP Request Duration | ${httpDuration.avg.toFixed(2)} | ${httpDuration.min.toFixed(2)} | ${httpDuration.med.toFixed(2)} | ${httpDuration.max.toFixed(2)} | ${httpDuration.p90.toFixed(2)} | ${httpDuration.p95.toFixed(2)} | ${httpDuration.p99.toFixed(2)} |

### Server-Side Latency Breakdown
| Phase | Avg (ms) | Med (ms) | P(95) (ms) | P(99) (ms) | Max (ms) |
|---|---|---|---|---|---|
| Session Lock Acquire | ${lockAcquire.avg.toFixed(2)} | ${lockAcquire.med.toFixed(2)} | ${lockAcquire.p95.toFixed(2)} | ${lockAcquire.p99.toFixed(2)} | ${lockAcquire.max.toFixed(2)} |
| Session State Load | ${stateLoad.avg.toFixed(2)} | ${stateLoad.med.toFixed(2)} | ${stateLoad.p95.toFixed(2)} | ${stateLoad.p99.toFixed(2)} | ${stateLoad.max.toFixed(2)} |
| Security Policy Eval | ${coreEval.avg.toFixed(2)} | ${coreEval.med.toFixed(2)} | ${coreEval.p95.toFixed(2)} | ${coreEval.p99.toFixed(2)} | ${coreEval.max.toFixed(2)} |
| Session State Save | ${stateSave.avg.toFixed(2)} | ${stateSave.med.toFixed(2)} | ${stateSave.p95.toFixed(2)} | ${stateSave.p99.toFixed(2)} | ${stateSave.max.toFixed(2)} |
| Total Server Time | ${serverTime.avg.toFixed(2)} | ${serverTime.med.toFixed(2)} | ${serverTime.p95.toFixed(2)} | ${serverTime.p99.toFixed(2)} | ${serverTime.max.toFixed(2)} |
`;
...
================================================================================
Target URL:       ${targetUrl}
Storage Tier:     ${report_json.summary.storage_type}
VUs / Concurrency: ${vus}
Total Requests:   ${httpReqs.count} (${httpReqs.rate.toFixed(2)} req/s)
Error Rate:       ${(errors.rate * 100.0).toFixed(2)}%
Client Duration:  Avg=${httpDuration.avg.toFixed(2)}ms, Med=${httpDuration.med.toFixed(2)}ms, P(95)=${httpDuration.p95.toFixed(2)}ms, P(99)=${httpDuration.p99.toFixed(2)}ms
Server Eval:      Avg=${coreEval.avg.toFixed(2)}ms, Med=${coreEval.med.toFixed(2)}ms, P(95)=${coreEval.p95.toFixed(2)}ms, P(99)=${coreEval.p99.toFixed(2)}ms
--------------------------------------------------------------------------------
...
`;
  ...
  return {
    ...
    [jsonPath]: JSON.stringify(report_json, null, 2),
    [mdPath]: md,
  };
}
