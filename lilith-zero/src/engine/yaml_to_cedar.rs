...
impl CedarCompiler {
    ...
    resource == Resource::"{}"
        ...
        for (i, rule) in def.resource_rules.iter().enumerate() {
            ...
            let cedar_pattern = rule.uri_pattern.replace("\\*", "*"); // Ensure no escaping of *
    ...
    context.path like "{}"
        ...
        // Add default permit for resources/read (since it's a negative security model for resources)
        let default_resource_permit = Policy::parse(
            Some(PolicyId::from_str("default_resource_permit").unwrap()),
            r#"permit(principal, action == Action::"resources/read", resource);"#,
        ).unwrap();
