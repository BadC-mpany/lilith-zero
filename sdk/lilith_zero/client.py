...
class Lilith:
    ...
    def __init__(
        ...
        *,
        ...
        if not upstream or not upstream.strip():
            raise LilithConfigError("Upstream command is required in this version.")
        ...
        if not os.path.exists(self._binary_path):
             raise LilithConfigError(f"Lilith binary not found at {self._binary_path}")
    ...
    async def _connect(self) -> None:
        ...
        try:
            ...
            raise LilithConnectionError(f"Failed to spawn Lilith: {e}")
    ...
    async def _wait_for_session(self) -> None:
        ...
        if self._process and self._process.returncode is not None:
             ...
             if self._process.stderr is None:
                 raise LilithProcessError(f"Lilith process exited early with code {self._process.returncode}")
             err = await self._process.stderr.read()
             raise LilithProcessError(
                f"Lilith process exited early with code {self._process.returncode}. Stderr: {err.decode()}"
            )
        raise LilithConnectionError("Timed out waiting for Lilith session ID")
            ...
            for future in self._pending_requests.values():
                if not future.done():
                    future.set_exception(
                        LilithProcessError("Lilith process terminated unexpectedly")
                    )
        ...
        if future and not future.done():
            if "error" in msg and msg["error"]:
                ...
                if "Policy Violation" in message or code == -32000:
                    ...
                    future.set_exception(LilithError(f"Lilith RPC Error: {message}"))
            else:
    ...
    async def _send_notification(self, method: str, params: Dict[str, Any]) -> None:
        if not self._process or not self._process.stdin:
            raise LilithConnectionError("Lilith process not running")
        ...
        async with self._lock:
            try:
                ...
                await self._process.stdin.drain()
                 ...
                 raise LilithConnectionError("Broken pipe to Lilith process")
